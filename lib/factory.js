...
function getProvider(provider) {
  console.log('log providers 2', provider);
  try {
    // Try to load the provider from providers folder
    console.log('from folder providers', require('./providers/' + provider));
    return require('./providers/' + provider);
  } catch (err) {
    console.log('error providers', provider);
    try {
      // Fall back to using an external provider
